...
int   flag_price = 0,        /* if = 1 - signal to start price-in ASAP - 
      ...
      flag_updt = 0,         /* if = 1 - update failed some sources are 
				...
				unfeasible or you have to return 
                                suspended arcs */
      flag_bk_arc = 1;   /* if 1 indicate push arc to front of excess */
...

{
  ...
  while (1)
  {
    ...
    if (j_exc >= 0)
    {
      ...
      {
        if (bk_arc==1 & flag_bk_arc==1){
          INSERT_TO_FRONT_EXCESS_Q(j);
          bk_arc==0;
        }
        else{
...

{
  ...
  while (1)
  {
    ...
    if (i->excess > 0)
    {
      ...
      if (flag_bk_arc==0 & (TIME_FOR_UPDATE || flag_price))
      {
        if (i->excess > 0)
        {
          INSERT_TO_EXCESS_Q(i);
...
//cc = 0;
  ...
  }while(++lp_cnt < lp_up_dound); /* end of main loop */
...
price_t* res_print(node *ndp, arc *arp, long nmin, double *cost)
{
  ...
  /*save trajectories*/
  price_t *tracks = (price_t*) calloc ( 2*n,   sizeof(price_t) ); 
  ...
  */
  ...
  for (f=0; f<j; f++)
	tracks_nonredundant[f+1] = tracks[f];
 ...
 *f_sc = msz[0];
 ...
 /*printf("(n,m,scale) = (%d,%d,%d)\n",n,m,*f_sc);*/
  ...
  /* Store excesses -- no need for circulation
 ...
 */ 
 ...
 for (i = 0; i < m; i++) {
   ...
   if ( low < 0 || low > acap )
     { printf("Lower bound exceeds upper boud");
	err_end(ALLOCATION_FAULT);
     }
...
price_t* pyCS2(long *msz, double *mtail, double *mhead, double *mlow, double *macap, double *mcost)
{
  ...
  */
  ...
  free(node_balance);
#endif
  //price_t* tmp = (long long int *) calloc (3, sizeof(long long int));
  //tmp[0] = 10;
  //tmp[1] = 20;
  //tmp[2] = 30;
  return track_vec;
