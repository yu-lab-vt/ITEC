...
% order_flag:1-second order and zero order information
%            0-zero order information             
if nargin == 9
    order_flag = true;
end
...
comMapsInit = get_local_area_Wei(vid, idMap, pseudo_seed_label(1), eig3d, eig2d, seed_yxz, q.shift);
...
if order_flag
    ...
    comMaps = fgDetectSynQuant(comMapsInit, q);
    if isnan(comMaps.pickedThreshold) || isempty(find(comMaps.fmapComp, 1))
        comMaps.fmapComp = false(size(comMaps.vidComp));
        return;
    end
    comMaps.init_seed_map = comMapsInit.regComp;
    % fprintf('%d \n',parent_kid_vec);
end